...
public class TerrainObjectsEditor : EditorWindow {
    ...
    public void OnGUI()
    {
       ...
       * */
        ...
        _popRate = EditorGUILayout.IntSlider("Trees Pop Rate", _popRate, 0, 100);
        EditorGUILayout.EndHorizontal();

        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("Terrain Width");
        _width = EditorGUILayout.IntSlider(_width, 0, 35000);
        EditorGUILayout.EndHorizontal();

        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("Terrain Depth");
        _depth = EditorGUILayout.IntSlider(_depth, 0, 35000);
        EditorGUILayout.EndHorizontal();
    ...
    void CreateTerrain()
    {
        ...
        if (_prefab != null)
        {
            ...
            {
                for (int j = 0; j < _depth; j+=2)
                {
                    if (Random.Range(0,100) < _popRate)
                    {
                        if(Random.Range(0,100) > 85)
                        {
                            GameObject pref = (GameObject)Instantiate(_prefab, new Vector3(i - _width / 2, 0, j - _depth / 2), Quaternion.identity);
                            pref.transform.Rotate(new Vector3(0, Random.Range(0, 180), 0));
                            pref.transform.localScale = new Vector3(1, Random.Range(-0.9f, 1.1f), 1);
                            instanciatedObjects.Add(pref.transform);
                        }
